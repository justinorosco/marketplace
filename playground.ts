...
document.querySelectorAll<HTMLButtonElement>(".c-alert-close").forEach((closeBtn) => {
  closeBtn.addEventListener("click", () => {
    const alert = closeBtn.closest<HTMLElement>(".c-alert");
    alert?.setAttribute("hidden", "");
  });
});

// ---------- Dialogs ----------
document.querySelectorAll<HTMLButtonElement>("[data-dialog-open]").forEach((openBtn) => {
  ...
  dialog.querySelectorAll<HTMLButtonElement>("[data-dialog-close]").forEach((closeBtn) => {
    closeBtn.addEventListener("click", () => {
      dialog.close();
    });
  });
...
// ---------- Menús ----------
const menuToggles = document.querySelectorAll<HTMLButtonElement>(".c-menu-account-toggle");

function closeAllMenus(except?: HTMLElement): void {
  document.querySelectorAll<HTMLElement>(".c-menu-account").forEach((menu) => {
...
menuToggles.forEach((toggle) => {
  ...
  if (!menuId) return;
  const menu = document.querySelector<HTMLElement>(`[data-menu="${menuId}"]`);
  if (!menu) return;
...
document.querySelectorAll<HTMLButtonElement>("[data-toast-demo]").forEach((btn) => {
